...
class Splitted_AlexNet(object):

    def __init__(self, x, keep_prob, num_classes, num_ch, ch_idx, skip_layer, weights_path='DEFAULT'):
    ...
    def create(self):
        ...
        conv1 = conv(self.X, self.CH_IDX, 11, 11, 96, 4, 4, padding='VALID', name='conv1')
        pool1 = max_pool(conv1, 3, 3, 2, 2, padding='VALID', name='pool1')
        norm1 = lrn(pool1, 2, 2e-05, 0.75, name='norm1')

        # 2nd Layer: Conv (w ReLu) -> Pool -> Lrn with 2 groups
        conv2 = conv(norm1, self.CH_IDX, 5, 5, 256, 1, 1, groups=2, name='conv2')
        pool2 = max_pool(conv2, 3, 3, 2, 2, padding='VALID', name='pool2')
        norm2 = lrn(pool2, 2, 2e-05, 0.75, name='norm2')

        # 3rd Layer: Conv (w ReLu)
        conv3 = conv(norm2, self.CH_IDX, 3, 3, 384, 1, 1, name='conv3')

        # 4th Layer: Conv (w ReLu) splitted into two groups
        conv4 = conv(conv3, self.CH_IDX, 3, 3, 384, 1, 1, groups=2, name='conv4')

        # 5th Layer: Conv (w ReLu) -> Pool splitted into two groups
        conv5 = conv(conv4, self.CH_IDX, 3, 3, 256, 1, 1, groups=2, name='conv5')
        pool5 = max_pool(conv5, 3, 3, 2, 2, padding='VALID', name='pool5')

        # 6th Layer: Flatten -> FC (w ReLu) -> Dropout
        # Here is the modification to multichannel: output 4096 -> 1024
        flattened = tf.reshape(pool5, [-1, 6*6*256])

        # fc6 = fc(flattened, 6*6*256, 4096, name='fc6')
        self.fc6 = fc(flattened, self.CH_IDX, 6*6*256, int(4096/self.NUM_CH), name='fc6')
